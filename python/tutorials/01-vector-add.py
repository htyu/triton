...
@triton.autotune(
    configs=[
        ...
        triton.Config({}, num_stages=16, num_warps=4),
        triton.Config({}, num_stages=32, num_warps=4),
    ],
    key=[],
)
@triton.jit
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    kernel_info = add_kernel[grid](x, y, output, n_elements, BLOCK_SIZE=BLOCK_SIZE, stride=512)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[2**30],  # Different possible values for `x_name`.
