...
def matmul_persistent(a, b):
    configs = {
        torch.float8_e4m3fn: {
            "BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 8, "num_stages": 4,
            "num_warps": 8
        }, torch.float16: {
            "BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 64, "GROUP_SIZE_M": 8, "num_stages": 3,
            "num_warps": 8
...
@triton.jit(launch_metadata=_matmul_launch_metadata)
def matmul_kernel_tma_persistent_2(a_desc_ptr, b_desc_ptr, c_ptr,  #
                                   M, N, K,  #
                                   stride_cm, stride_cn,  #
                                   BLOCK_SIZE_M: tl.constexpr,  #
                                   BLOCK_SIZE_N: tl.constexpr,  #
...
def matmul_tma_persistent_2(a, b):
    # Autotuner does not work with TMA. Use manual config.
    configs = {
        torch.float8_e4m3fn: {
            "BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 8, "num_stages": 4,
    ...
    grid = lambda META: (min(NUM_SMS, triton.cdiv(M, META["BLOCK_SIZE_M"]) * triton.cdiv(N, META["BLOCK_SIZE_N"])), )
    matmul_kernel_tma_persistent_2[grid](
        desc_a, desc_b, c,  #
        M, N, K,  #
        c.stride(0), c.stride(1),  #
        BLOCK_SIZE_M=configs[dtype]["BLOCK_SIZE_M"],  #
...
def validate(M, N, K, dtype):
    ...
    tma_persistent_result_2 = matmul_tma_persistent_2(a, b) if supports_tma() else None

    if tma_persistent_result is not None:
        persistent_vs_tma = "✅" if torch.allclose(persistent_result.to(torch.float16),
                                                  tma_persistent_result.to(torch.float16), atol=1.0) else "❌"
    if tma_persistent_result_2 is not None:
        persistent_vs_tma_2 = "✅" if torch.allclose(persistent_result.to(torch.float16),
                                                    tma_persistent_result_2.to(torch.float16), atol=1.0) else "❌"

    print(f"M={M}, N={N}, K={K} verification persistent vs: ", end="")

    if persistent_vs_tma is not None:
        print(f"TMA persistent: {persistent_vs_tma}")

    print(f"M={M}, N={N}, K={K} verification persistent vs: ", end="")

    if persistent_vs_tma_2 is not None:
        print(f"TMA persistent 2: {persistent_vs_tma_2}")

    torch.set_printoptions(profile="full")
    # print(persistent_result)
    # print(tma_persistent_result)
