...
def cublas_matmul(a, b):
    ...
    with proton.scope(
        ...
        {"bytes": bytes_per_elem * (M * K + N * K), flops_str: 2.0 * M * N * K},
    ):
        cublas.matmul(a, b, c)
    return c
...
def validate(M, N, K, dtype):
    ...
    cublas_vs_tma_persistent = ("✅" if torch.allclose(
        cublas_result.to(torch.float16),
        tma_persistent_result.to(torch.float16),
        atol=1.0,
    ) else "❌")
    print(
        f"M={M}, N={N}, K={K} verification cublas vs TMA persistent: {cublas_vs_tma_persistent}",
        end="",
    )
    print()
    # torch.set_printoptions(threshold=torch.inf)
    # print(f"cublas_result: {cublas_result}")
    # print(f"tma_persistent_result: {tma_persistent_result}")
...
if __name__ == "__main__":
    ...
    validate(256, 512, 2048, dtype)

    # validate(8192, 8192, 32, dtype)
